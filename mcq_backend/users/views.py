...
from .serializers import CustomUserSerializer,SetNewPasswordSerializer, ResetPasswordEmailRequestSerializer,UpdateUserSerializer,ProfileSerializer,TempUserSerializers,ContactSerializer
from rest_framework_simplejwt.tokens import RefreshToken
from rest_framework.permissions import AllowAny
from rest_framework import viewsets, filters, generics, permissions
from .models import NewUser,TempUser,Contact
...
class TempUserView(generics.RetrieveUpdateDestroyAPIView):
    permission_classes = [AllowAny]
    serializer_class = TempUserSerializers
    queryset=TempUser.objects.all()

class TempUserView1(generics.CreateAPIView):
    permission_classes = [AllowAny]
    serializer_class = TempUserSerializers
    queryset=TempUser.objects.all()
class TempUserView2(generics.ListAPIView):
    permission_classes = [AllowAny]
    serializer_class = TempUserSerializers
    queryset=TempUser.objects.all()
    def get_queryset(self,**kwargs):
        pk=self.kwargs.get('pk')
        abcd = TempUser.objects.filter(rel_key=pk)
        print(abcd)
        return abcd
